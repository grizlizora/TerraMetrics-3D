...
export const AppPreloader: React.FC<AppPreloaderProps> = ({
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#f0f4f8] dark:bg-[#060a12] p-6 select-none transition-all duration-500 ease-in-out ${
        isFadingOut ? 'opacity-0 pointer-events-none scale-105 backdrop-blur-none' : 'opacity-100 scale-100'
      }`}
    >
      <LiquidGlassPanel
        intensity="high"
        className="relative p-8 max-w-sm w-full text-center space-y-6 shadow-2xl rounded-3xl border border-black/10 dark:border-white/10"
      >
        {/* Language switcher top right */}
        <button
          onClick={toggleLang}
          className="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/5 dark:bg-white/10 text-zinc-700 dark:text-zinc-300 text-xs font-semibold hover:bg-black/10 dark:hover:bg-white/20 transition-colors cursor-pointer"
        ...
        <div className="relative w-16 h-16 mx-auto flex items-center justify-center">
          ...
          <div className="w-16 h-16 flex items-center justify-center rounded-2xl bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 backdrop-blur-md border border-blue-500/20 dark:border-blue-500/30">
            {stageKey === 'error' ? (
              <AlertCircle className="w-8 h-8 text-rose-500" />
            ) : (
              <Globe className="w-8 h-8 animate-pulse text-blue-600 dark:text-blue-400" />
            )}
          </div>
        </div>
        ...
        {stageKey !== 'error' ? (
          <div className="space-y-2.5 pt-2">
            <div className="flex justify-between items-center text-xs font-semibold px-1">
              <span className="text-zinc-700 dark:text-zinc-300 text-left truncate max-w-[200px]">
                {getStageTitle()}
              </span>
              <span className="text-blue-600 dark:text-blue-400 font-mono text-xs font-bold">{progress}%</span>
            </div>
